...
describe('services/Typeform.js', () => {
  ...
  const apiUrl = process.env.TYPEFORM_URL;
  ...
  describe('Get Files', () => {
    ...
    beforeEach(async () => {
      if (fileUrl) return;
      const responses = await Typeform.getResponses(apiUrl, {formId, token, query: {page_size: 1}});
