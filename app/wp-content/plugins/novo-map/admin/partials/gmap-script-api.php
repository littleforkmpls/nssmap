<script type="text/javascript">
    function initialize_<?php echo esc_js($gmap_name) ?>() {
        ...
        //add click event on admin to easily fill latitude and longitude
        google.maps.event.addListener(<?php echo esc_js($gmap_name) ?>, 'click', function( event ){
            var latInput = document.getElementById('<?php echo esc_js($gmap_name) ?>-latitude');
            var lngInput = document.getElementById('<?php echo esc_js($gmap_name) ?>-longitude');
            setReactInputValue(latInput, event.latLng.lat().toFixed(6));
            setReactInputValue(lngInput, event.latLng.lng().toFixed(6));
        });
        //to dispatchEvent on input change --> https://github.com/facebook/react/issues/10135#issuecomment-500929024
